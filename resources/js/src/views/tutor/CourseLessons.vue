    ...
    <div class="portal-page-grid">
        ...
        <template v-else-if="course">
            <PageHeader
                ...
                :description="t('routes.tutor.lessonsSubtitle')"
            >
                <template #actions>
                    <RouterLink
                        :to="{ name: 'tutor-course-edit', params: { id: course.id } }"
...
async function submit(values) {
    ...
    try {
        const payload = await portalApi.createLessonForCourse(route.params.id, {
            titre: values.title,
            type: values.type,
            file: values.file,
        });
